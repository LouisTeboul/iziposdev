app.service('productService', ['$rootScope', '$q', 'settingService', 'taxesService',
    function ($rootScope, $q, settingService, taxesService) {
        ...
        /** Get Product by id 
        *  CAUTION: The function do more than it is specified is in name
        * */
                            ...
                            if (values.LinkedProducts) {
                                for (var k = 0; k < productAttribute.ProductAttributeValues.length; k++) {
                                    ...
                                    if (linkedProduct) {
                                        ...
                                        //Obtains VAT for linkedProduct
                                        if (linkedProduct.TaxCategoryId) {
                                            var taxCategory = Enumerable.from(values.TaxCategories).firstOrDefault('x => x.TaxCategoryId === ' + linkedProduct.TaxCategoryId);
                                            if (taxCategory) {
                                                linkedProduct.TaxCategory = taxCategory;
                                            }
                                        }
